...
class _EventCreatorState extends State<EventCreator> {
  ...
  changeStartDate(DateTime startsAt) {
    DateTime startTime = startsAt;
    DateTime endTime = event.endsAt as DateTime;
    if (!isSameDay(startsAt, event.startsAt)) {
      startTime = DateTime(
          startsAt.year,
          startsAt.month,
          startsAt.day,
          (event.startsAt as DateTime).hour,
          (event.startsAt as DateTime).minute);
      endTime = DateTime(startTime.year, startTime.month, startTime.day,
          (event.endsAt as DateTime).hour, (event.endsAt as DateTime).minute);
    }
    setState(() {
      event = Event(
          description: event.description,
  ...
  changeStartTime(DateTime startTime) {
    DateTime endTime = event.endsAt as DateTime;
  ...
  changeEndTime(DateTime endTime) {
    if (endTime.isAfter(event.startsAt as DateTime)) {
      setState(() {
        event = Event(
            description: event.description,
            uid: event.uid,
  ...
  Widget build(BuildContext context) {
    var datePicker = DatetimePicker(
      ...
      initDate: event.startsAt as DateTime,
      time: false,
      toggleChangeDatetime: changeStartTime,
    );

    var startTimePicker = DatetimePicker(
      minDate: isSameDay(event.startsAt, DateTime.now())
          ? DateTime.now()
          : DateTime(
              (event.startsAt as DateTime).year,
              (event.startsAt as DateTime).month,
              (event.startsAt as DateTime).day,
              0,
              0),
      maxDate: DateTime(
          (event.startsAt as DateTime).year,
    ...
    var endTimePicker = DatetimePicker(
      ...
      maxDate: DateTime(
          ...
          23,
          59),
      initDate: event.endsAt,
      time: true,
      toggleChangeDatetime: changeEndTime,
    );
